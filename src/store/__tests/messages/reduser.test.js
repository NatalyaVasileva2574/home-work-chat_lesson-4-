...
describe("message reducer", () => {
  it("send message", () => {
    ...
    expect(result.messages.room1[1].message).toBe(MESSAGE.message);
  });
  ...
  it("delete message", () => {
    ...
    expect(result.messages.room1.filter(m => m.id === MESSAGE_ID).length
    ).toBe(0);
  });
  ...
  describe('get messages types', () => {
    ...
    it("error", () => {
      ...
      expect(result.error).toBeFalsy(ERROR);
    });
  })
});